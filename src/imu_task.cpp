...
static void bring_up(mti::Uart &uart, xbus::Parser &parser) {
  while (true) {
    ...
    if (!wait_for(uart, parser, xbus::MID::GoToConfigAck, ACK_TIMEOUT_MS)) {
      if (Serial) Serial.println("# IMU: GoToConfig timeout, retrying");
    ...
    if (!wait_for(uart, parser, xbus::MID::OutputConfigAck, ACK_TIMEOUT_MS)) {
      if (Serial) Serial.println("# IMU: SetOutputConfig timeout, retrying");
      vTaskDelay(pdMS_TO_TICKS(RETRY_DELAY_MS));
      continue;
    }
    ...
    if (!wait_for(uart, parser, xbus::MID::GoToMeasAck, ACK_TIMEOUT_MS)) {
      if (Serial) Serial.println("# IMU: GoToMeasurement timeout, retrying");
      vTaskDelay(pdMS_TO_TICKS(RETRY_DELAY_MS));
      continue;
    }

    if (Serial) Serial.println("# IMU: ready");
    return;
  }
}
