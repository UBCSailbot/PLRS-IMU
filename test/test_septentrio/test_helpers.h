 ...
 *        Body is taken verbatim; caller is responsible for any padding to
 *        make length a multiple of 4.
 *
 * Lets the caller force a specific length field, which may mismatch
 * body.size(). Used to test the parser's length-validation paths (not
 * multiple of 4, < HEADER_LEN, > MAX_BLOCK).
 *
 * @param id      Full 16-bit ID field (block number in bits 0-12,
 *                revision in bits 13-15).
 * @param length  Value to write into the Length header field.
...
/**
 * @brief Assemble an SBF block with the Length field set to match body.size().
 *        The common case; thin wrapper over make_block_with_length.
 */
inline std::vector<uint8_t> make_block(uint16_t id,
                                       const std::vector<uint8_t> &body) {
  const uint16_t length = static_cast<uint16_t>(sbf::HEADER_LEN + body.size());
  return make_block_with_length(id, length, body);
}
